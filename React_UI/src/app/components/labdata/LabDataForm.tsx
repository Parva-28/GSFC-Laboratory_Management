...
export default function LabDataForm({ user, onNavigate, onLogout }: LabDataFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post("labdata/save/", {
        tanker_id: formData.tankerId,
      ...
      setStatusMsg({
        type: "error",
        text: `❌ ${err.response?.data?.error || err.message || "Server error"}`
      });
    } finally {
      setIsSaving(false);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-gray-700 font-medium mb-2">
                  Tanker ID (From Arrival) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  name="tankerId"
